...
class Market extends Component {
  ...
  async getArray() {
    var objectArr = []
    let listIdNFT = await this.state.myNFT.methods.getListIdNFT().call({ from: this.state.account })
  ...
  async getEvent(){
   var events = this.state.myNFT.once('MyEvent', {
          ...
          <div className="contact-box center-version">
            <a href="#">
              ...
              <address className="m-t-md">
                <strong>Price: {window.web3.utils.fromWei(nft.price)} GOLD</strong><br />
                {nft.description}<br />
              </address>
            </a>
